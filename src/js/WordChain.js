...
class WCengine {
  ...
  winWord2(char) {
    let mcts = new MCTS(new MCTSTurn(undefined, char, this, []), 1000)
    return mcts
  }
  remove_hanbang() {
  ...
  exhaustive(char) {
    ...
    const turn = Turn(undefined, char, this, [])
...
class MCTSTurn extends Turn {
  ...
  getTurnFromRoute(route) {
    ...
    if (breaking) {
      ...
      nextTurn = new MCTSTurn(route, this.WCengine.rule.tail(route), nextWm, [])
    }
    else {
      nextTurn = new MCTSTurn(route, this.WCengine.rule.tail(route), this.WCengine, [...this.except, route])
    }
...
export { LOS, WIN, LOSCIR, WINCIR, ROUTE, Rule, WCengine, MCTSTurn, MCTS, Turn }
