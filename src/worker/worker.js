import { WCengine, Rule, MCTS, MCTSTurn, Turn,WIN, LOS, WINCIR, LOSCIR, ROUTE} from "../js/WordChain"
...
const analysisMCTS = (data)=>{
  ...
  if(heuristicWord && heuristicWord.win !== "ROUTE"){
    ...
    self.postMessage({trail : [],chart : {[heuristicWord.word] : heuristicWord.win ? 1 : 0}, num : 0, terminate : true}) 
...
const analysisExhaustive = (data) => {
  ...
  if(heuristicWord && heuristicWord.win !== "ROUTE"){
    ...
    self.postMessage({word : heuristicWord.word, win : heuristicWord.win, terminate : true}) 
  ...
  const nextRoute = currTurn.nextRoute.sort((a, b) =>
