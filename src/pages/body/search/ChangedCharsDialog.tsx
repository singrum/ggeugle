  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[80vh] min-h-0 overflow-auto  scrollbar-thin">
        <DialogHeader>
          <DialogTitle>유형이 변경된 글자</DialogTitle>
        ...
        <div className="flex gap-1 justify-center">
          {changeTypeList.map(({ name }, i) => (
            <div
              key={i}
              className={cn(
                "py-1 px-3 rounded-full text-muted-foreground hover:text-foreground hover:font-medium cursor-pointer text-sm transition-colors select-none",
                { "bg-accent text-foreground": i === changeType }
              )}
              onClick={() => setChangeType(i)}
            >
              {name}
            </div>
          ))}
