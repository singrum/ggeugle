...
import { mergedMeta, metaDescription, metaTitle } from "~/lib/utils";
...
export const meta: MetaFunction = ({ params, matches, loaderData }) => {
  return mergedMeta(matches, [
    ...metaTitle(
      `${(params["*"] ?? "지식").replace("/", " - ")} | 끝말잇기 엔진`,
    ),
    ...metaDescription(
      `${(loaderData as { data: string }).data
        .replace(/^#\s+/, "")
        .slice(0, 100)}...`,
    ),
  ]);
